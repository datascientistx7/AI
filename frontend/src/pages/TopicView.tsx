...
import { topics, modes, modules, getTopicById } from '../lib/data';
import { motion } from 'framer-motion';
import { Sparkles, FlaskConical, BrainCircuit, Columns, GraduationCap, ChevronLeft, CheckCircle2, Target } from 'lucide-react';
...
export const TopicView = () => {
  ...
  const { getModeProgress, getTopicProgress } = useProgress();

  const modTopics = topics[moduleId || ''] || [];
  const topic = modTopics.find((t) => t.id === topicId) || getTopicById(topicId || '');
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-6xl mx-auto py-10 space-y-12">
      ...
      <div className="text-center max-w-2xl mx-auto">
        ...
        <div className="mt-6 inline-flex items-center gap-4 rounded-full border border-white/10 bg-surface/60 px-5 py-3 text-sm text-gray-300">
          <span>{topicProgress.completedModes}/{topicProgress.totalModes} modes complete</span>
          <span className="w-1.5 h-1.5 rounded-full bg-white/30" />
          <span>{topicProgress.percent}% mastery</span>
          {topicProgress.examBestScore !== null && (
            <>
              <span className="w-1.5 h-1.5 rounded-full bg-white/30" />
              <span>Best exam: {topicProgress.examBestScore}%</span>
            </>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
        {modes.map((mode, idx) => {
          ...
          const modeProgress = getModeProgress(topic.id, mode.type);
            ...
            <motion.div key={mode.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.1 }} className="h-full">
              <Link to={`/dashboard/modules/${moduleId}/${topicId}/mode/${mode.type.toLowerCase()}`} className="block h-full">
                <div className="glass-card hover:-translate-y-2 transition-all p-6 text-center space-y-4 h-full flex flex-col items-center group relative overflow-hidden">
                  ...
                  <div className="w-full pt-4 border-t border-white/5 mt-4 z-10 text-xs text-gray-500 space-y-2">
                    <div>
                      Character: <span className="capitalize">{mode.character}</span>
                    </div>
                    {mode.type === 'Exam' && modeProgress?.bestScore !== null && (
                      <div className="inline-flex items-center gap-1 text-characters-gojo">
                        <Target className="w-3.5 h-3.5" />
                        <span>Best score {modeProgress?.bestScore}%</span>
                      </div>
                    )}
                    {mode.type !== 'Exam' && modeProgress?.completed && <div className="text-success">Completed</div>}
                  </div>
                </div>
              </Link>
            </motion.div>
